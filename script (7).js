...
document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            if (gameMode === 'online') {
                localStorage.setItem(SAVE_KEY, JSON.stringify({
                    ...
                    boardSize: boardSize,
                    mySymbol: myOnlineSymbol
                }));
                console.log('[SAVE] Online saved:', currentRoomCode, 'symbol:', myOnlineSymbol);
                ...
                console.log('[SAVE] Local saved, turn:', localCurrentPlayer);
            }
        } catch (e) {
    ...
    function restoreGameState() {
        ...
        console.log('[RESTORE] raw:', raw ? 'found' : 'not found');
        if (!raw) return false;
        try {
            const state = JSON.parse(raw);
            console.log('[RESTORE] state:', state.gameMode, 'active:', state.gameActive, 'room:', state.roomCode);
            if (state.gameMode === 'online' && state.roomCode) {
                currentRoomCode = state.roomCode;
                gameMode = 'online';
                boardSize = state.boardSize || 3;
                myOnlineSymbol = state.mySymbol || null;
                connectRoom(state.roomCode);
                initBoard(boardSize);
                showScreen('game');
                console.log('[RESTORE] Online restored, symbol:', myOnlineSymbol);
                return true;
                ...
                updateTurnText();
                showScreen('game');
                console.log('[RESTORE] Local restored, turn:', localCurrentPlayer);
                return true;
            }
    ...
    setBoardCallback((room) => {
        ...
        if (gameMode === 'online') {
            ...
            } else if (!myOnlineSymbol && state.mySymbol) {
                myOnlineSymbol = state.mySymbol;
            }
    ...
    document.getElementById("create-room").addEventListener("click", async () => {
        try {
            ...
            gameMode = "online";
            myOnlineSymbol = 'X'; // Le créateur joue X
            connectRoom(code);
    ...
    async function doJoinRoom(code) {
        try {
            ...
            currentRoomCode = code;
            gameMode = "online";
            myOnlineSymbol = 'O'; // Le rejoignant joue O
            connectRoom(code);
            showScreen('game');
        } catch (err) {
